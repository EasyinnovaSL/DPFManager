package dpfmanager.shell.jacp.application.launcher.noui;

import dpfmanager.shell.jacp.core.config.ConsoleConfig;
 ...
 */
public class NoUiWorkbench implements Base<Node, EventHandler<Event>, Event, Object>, RootComponent<Perspective<Node, EventHandler<Event>, Event, Object>, Message<Event, Object>> {
  ...
  public NoUiWorkbench(final FXWorkbench handler, final WorkbenchDecorator decorator){
    this.setComponentHandle(handler);
    this.setWorkbenchDecorator(decorator);
  }

  @Override
  public void removeAllCompnents() {
  ...
  private void initWorkbenchHandle(final Stage stage) {
    ...
    handle.handleInitialLayout(new MessageImpl(context.getId(), "init"), getWorkbenchLayout(), stage);
    initWorkbenchDecorator(stage);
    handle.postHandle(new FXComponentLayout(getWorkbenchLayout().getMenu(), workbenchDecorator.getGlassPane(), null, getContext().getId()));
  }
...
//    });
  ...
  private void initSubsystem() {
    componentHandler = new PerspectiveHandlerImpl(launcher, workbenchLayout, workbenchDecorator.getRoot());
...
//    stage.show();
  ...
  public void unregisterComponent(final Perspective<Node, EventHandler<Event>, Event, Object> perspective) {
