...
import dpfmanager.shell.interfaces.Old.Gui;
...
public class OldApp extends Application {
  private static final Logger LOG = LoggerFactory.getLogger(OldApp.class);
  ...
  private String getDefaultBrowseDirectory() {
    return DPFManagerProperties.getDefaultDir();
  }

  private void CreateDefaultConfigurationFiles() {
    File file = new File(DPFManagerProperties.getConfigDir() + "/Baseline HTML.dpf");
    if (!file.exists()) {
      try {
  ...
  private void LoadGui(boolean noDisc) throws Exception {
    ...
    if (!noDisc && DPFManagerProperties.getFirstTime()) {
      ShowDisclaimer();
